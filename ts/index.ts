...
client.on("ready", async () => {
  ...
  // @ts-ignore
  const events: EventFile[] = await ReadDirectoryFiles(
    files,
    join(client.__dirname, "clientEvents")
  );
  ...
  // @ts-ignore
  const commands: Command[] = await ReadDirectoryFiles(
    files,
    join(client.__dirname, "clientCommands")
  );
    ...
    if (fStat.isDirectory()) {
      commands = [
        ...
        ...(await ReadDirectoryFiles(
          await readdir(join(fullDir, file)),
          join(fullDir, file)
        )),
      ];
    } else {
      console.log(fullDir);
      commands.push(
        (await import(pathToFileURL(join(fullDir, file)).toString())).default
      );
    }
  }
  return commands;
