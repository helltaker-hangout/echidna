...
module.exports = {
  ...
  run: async function (client: _Client, message: Message, args: string[]) {
    if (message.member?.roles.cache.has('931329640914616320')) {
      ...
      message.mentions.members?.forEach(async (member) => {
        ...
        await member.roles.remove(catRole, logMsg);
        // @ts-ignore
        let channel: TextChannel = await client.guilds.cache.get(message.guild?.id).channels.fetch(logChan); //.then(channel => channel.send(logMsg));
        channel?.send(logMsg);
